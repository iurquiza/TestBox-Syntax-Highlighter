export default function(gulp, rootPath) {
  ...
  const loadReposFromCache = () =>
    fs.readFileAsync(REPOS_CACHE, "utf8").then(JSON.parse);
  const loadRepos = () =>
    loadReposFromCache()
      .error(loadReposFromGitHub)
      .then(R.map(R.pick(["clone_url", "name"])));
  const cloneRepo = repo => git(`clone ${repo.clone_url}`, REPOS_DIR);
  const pathToRepo = repo => `${REPOS_DIR}/${repo.name}`;
  const ln = (source, dest) =>
    rmfr(dest).then(() => exec(`ln -s ${source} ${dest} || true`));
  const linkNodeModulesIntoRepos = repo =>
    ln(`${rootPath}/node_modules`, `${pathToRepo(repo)}/node_modules`);
  const linkReposIntoNodeModules = repo =>
    ln(pathToRepo(repo), `${rootPath}/node_modules/${repo.name}`);
  const unlinkReposFromNodeModules = repo =>
    fs.promise.unlink(`${rootPath}/node_modules/${repo.name}`);

  gulp.task(
    "setup-project:clone-repos",
    "Clones all repositories from SyntaxHighlighter GitHub organization",
    () =>
      loadRepos()
  ...
  gulp.task(
    ...
    "Links `./node_modules` into every cloned repository",
    ["setup-project:clone-repos"],
    () =>
      loadRepos()
        .then(R.filter(repo => repo.name !== "syntaxhighlighter"))
  ...
  gulp.task(
    ...
    "Links every cloned repository into `./node_modules`",
    ["setup-project:clone-repos"],
    () =>
      loadRepos()
        .then(R.filter(repo => repo.name !== "syntaxhighlighter"))
  ...
  gulp.task(
    "setup-project:unlink-repos-from-node_modules",
    "Unlinks every cloned repository from `./node_modules`",
    () =>
      loadRepos()
        .then(R.filter(repo => repo.name !== "syntaxhighlighter"))

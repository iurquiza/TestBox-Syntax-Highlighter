...
function getVersion(rootPath) {
  ...
  return fs
    .readFileAsync(`${rootPath}/package.json`)
    .then(JSON.parse)
...
function getBuildBrushes(rootPath, argv, availableBrushes) {
  ...
  if (buildBrushes.length === 0 || buildBrushes[0] === "true") {
    return Promise.reject(
      new BuildError('Please specify at least one brush or "all".')
    );
  }
        ...
        fs
          ...
          .then(() =>
            fs.promise
              ...
              .then(
                () =>
                  (requirePath = path.relative(`${rootPath}/src`, requirePath))
              )
          )
          ...
          .catch(function() {
            ...
            return fs
              .readFileAsync(
                `${rootPath}/repos/brush-${name}/sample.txt`,
                "utf8"
              )
              .then(content => (sample = content))
              .catch(() => null);
...
function buildJavaScript(rootPath, outputPath, buildBrushes, version, compat) {
  ...
  const registerBrushes = render(
    `${rootPath}/build/templates/bundle-register-brushes.js.ejs`,
    { buildBrushes }
  );
...
function buildCSS(rootPath, outputPath, theme, version) {
  ...
  return fs.promise
    ...
    .then(data =>
      sass.promise.render({
        data,
        includePaths: [`${rootPath}/node_modules/theme-base`]
      })
    )
    .then(results =>
      fs.promise.writeFile(`${outputPath}/theme.css`, results.css)
    );
}
...
function copyHtml(rootPath, outputPath, buildBrushes, version) {
  ...
  return fs.promise.writeFile(
    ...
    render(`${rootPath}/build/templates/index.html.ejs`, {
      buildBrushes,
      version
    })
  );
  ...
  return Promise.all([
    ...
    argv =
      ...
      require("yargs")
        .describe(
          "brushes",
          "Comma separated list of brush names or paths to be bundled."
        )
    ...
    return getBuildBrushes(rootPath, argv, availableBrushes).then(function(
      buildBrushes
    ) {
      return Promise.all([
        buildJavaScript(
    ...
    function(done) {
      ...
      bundle(rootPath, null)
        .then(function({ theme, stats, buildBrushes }) {
          ...
          if (buildBrushes)
            gulpUtil.log(
              `Brushes: ${buildBrushes.map(brush => brush.name).join(", ")}`
            );
        ...
        .catch(err =>
          gulpUtil.log(
            gulpUtil.colors.red(err.isBuild ? err.message : err.stack)
          )
        );
    }
  );
